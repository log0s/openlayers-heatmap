 ...
 */
 ...
 // Modified to allow layer groups with different colors by Ryan Herman <https://github.com/log0s/>
 // Now requires color.js for parsing input colors into gradient stops
...
// Sub layer within a LayerGroup, contains points of a single type
Heatmap._SubLayer = OpenLayers.Class(Heatmap.Layer, {
  // ID of parent layer group canvas, used for events
  parentCanvasId: null,

  // Custom gradient stops based off input color
  gradientStops: null,

  // Contstructor: Heatmap.SubLayer
  // Used internally by Heatmap.LayerGroup for tracking points of a single type
  initialize: function(name, points, options, canvasId) {
    OpenLayers.Layer.prototype.initialize.apply(this, arguments);
    this.points = [];
    this.cache = {};
    this.canvas = document.createElement('canvas');
    this.parentCanvasId = canvasId;
    this.defaultRadius = 20;
    this.defaultIntensity = 0.2;
    this.defaultGradientStops = {
      0.00: 0xffffff00,
      0.10: 0x99e9fdff,
      0.20: 0x00c9fcff,
      0.30: 0x00e9fdff,
    ...
    if (options && options.baseColor) {
      ...
      this._generateGradientStops(options.baseColor);
    }
    this._addPoints(points);
    this.setGradientStops(this.gradientStops || this.defaultGradientStops);
  },
  ...
  moveTo: function(bounds, zoomChanged, dragging) {
    ...
    // // Trigger event on parent canvas so layer group knows to add the sub-canvas data to the main canvas
    // var event = new CustomEvent('layerDrawn', { 
    //   detail: {
    //     layerName: this.name
    //   } 
    // });
    // var parentCanvas = document.getElementById(this.parentCanvasId);
    // parentCanvas.dispatchEvent(event);
  },

  // Internal method to parse and add all passed points
  _addPoints: function(points) {
    points.forEach(function(point) {
      this.addSource(new Heatmap.Source(point));
    }, this);
  },

  _generateGradientStops: function(baseColor) {
    var baseHex = Color(baseColor).hexString().replace(/#/, '');

    this.gradientStops = {
      0.00: parseInt(baseHex + '00', 16),
      1.00: parseInt(baseHex + '88', 16)
    };
  },

  CLASS_NAME: 'Heatmap._SubLayer'
});

Heatmap.LayerGroup = OpenLayers.Class(Heatmap.Layer, {
  // Default gradient settings
  defaultGradientStops: null,

  // // Layers within the group, used internally for tracking
  // _layers: null,

  // List of layers, used to add layers to map
  layers: null,

  // Constructor: Heatmap.LayerGroup
  // Used to render multiple layers with different parameters in single canvas
  // Pass layer name, OL options, and array of objects with (at minimum) source points and options if desired
  // REQUIRED: Name and layers
  // USAGE:
  // var heatLayer = new Heatmap.LayerGroup('HeatLayer', {}, [
  //   {
  //     points: [ OpenLayers.LonLat, OpenLayers.LonLat ],
  //     options: {
  //       baseColor: '#fff'
  //     }
  //   },
  ...
  //   }
  // ]);
  initialize: function(name, options, layers) {
    OpenLayers.Layer.prototype.initialize.apply(this, arguments);
    this.points = [];
    this.layers = [];
    // this._layers = {};
    ...
    this.canvas = document.createElement('canvas');
    this.canvas.style.position = 'absolute';
    this.canvas.id = 'heat-layer-group-' + name;

    // Allow blending on the main canvas in order to mix multiple layer gradients together
    ...
    layers.forEach(function(layer, index) {
      var layerName = this.name + index;
      var layer = new Heatmap._SubLayer(layerName, layer.points, layer.options, this.canvas.id);
      // this._layers[layerName] = layer;
      this.layers.push(layer);
    }, this);

    // this.canvas.addEventListener('layerDrawn', function(event) {
    //   this._addCanvasLayer(event);
    // }.bind(this));
  },

  // Override Heatmap.Layer moveTo to avoid default drawing functionality
  moveTo: function(bounds, zoomChanged, dragging) {
    // The code is too slow to update the rendering during dragging.
    if (dragging)
      return;
  ...
  setMap: function(map) {
    ...
    // Manually add the sub layers to the map when the parent is added
    map.addLayers(this.layers);
  },

  removeMap: function(map) {
    OpenLayers.Layer.prototype.removeMap.apply(this, arguments);
    // Clean up the sub layers when removing the main layer
    this.layers.forEach(function(layer) {
      map.removeLayer(layer);
    });
  },

  // _addCanvasLayer: function(event) {
  //   var context = this.canvas.getContext('2d');
  //   var layer = this._layers[event.detail.layerName]

  //   context.drawImage(layer.canvas, 0, 0, this.canvas.width, this.canvas.height);
  // },
